...
def do_vg_evaluation(
    ...
    relation_mAP = True  # 是否评估关系mAP，即只在至少有一个关系的框上计算mAP
    ...
    num_rel_category = num_rel_classes
    ...
    if "relations" in iou_types:
        for k in sampling_space:
            ...
            if "relations" in iou_types:
                ...
                # global_container['result_dict'] = result_dict 的数据填充在此进行
                i = 0
        ...
        if len(sampling_space) > 1:
            ...
            for metric in metrics:
                ...
                # 使用scipy interp1d进行插值，处理数据不单调递增的情况
        ...
        # 显示每个指标的最优top-k
        if len(sampling_space) > 1:
            logger.info('每个指标的最优top-k:')
            for metric, optimum in optimum_per_metric.items():
                logger.info(f'{metric}: top-{optimum["index"]} 值为 {optimum["value"]}')

        metric_to_optimize = "f1_score"
        # 使用最优的top-k重新评估
        if len(sampling_space) > 1:
            logger.info(f'为指标优化: {metric_to_optimize}')
            top_k = optimum_per_metric[metric_to_optimize]["index"]
            logger.info(f'最优top-k: {top_k}')
            ...
            if "eval_f1_score" in evaluator.keys():
                evaluator['eval_f1_score'].calculate(global_container['result_dict'], None, mode)
                result_str += evaluator['eval_f1_score'].generate_print_string(mode)
            result_str += '=' * 100 + '\n'
    ...
    if "relations" in iou_types:
        if output_folder:
            ...
            out_file = os.path.join(output_folder, f'eval_results_top_{top_k}.json')
